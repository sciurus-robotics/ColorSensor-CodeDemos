...
async def print_loop():
    while True:
        ...
        r, g, b = await color_sensor.calibrated()
        hsv = await color_sensor.hsv()
        status = await color_sensor.calibration_status()
        print("raw %5d %5d %5d %5d | calibrated RGB %3d %3d %3d | h %3d s %3d v %3d | %s | %s"
              % (raw[0], raw[1], raw[2], raw[3], r, g, b, hsv.h, hsv.s, hsv.v, await color_sensor.color(), status))
...
async def control_loop():
    ...
    while True:
        ...
        if Button.RIGHT in pressed:
            ...
            if await color_sensor.calibration_status() == "calibrating":
                await color_sensor.calibrate(False)
                mins, maxs, profile, valid, applicable, visited = await color_sensor.calibration()
